# FILE: recruiter_outreach/ingestion/pdf_extract.py
    ...
    with pdfplumber.open(file_path) as pdf:
        for page_num, page in enumerate(pdf.pages):
            ...
            if table and len(table) > 1:
                ...
                logger.debug(
                    f"Page {page_num + 1}: no structured table, queued for text/LLM extraction."
                )
...
def _llm_extract(text: str, api_key: Optional[str]) -> list[dict]:
    try:
        ...
        logger.error("anthropic package not installed. Run: pip install 'recruiter-outreach[llm]'")
        return []

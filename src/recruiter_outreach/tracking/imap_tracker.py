# FILE: src/recruiter_outreach/tracking/imap_tracker.py
...
Run periodically (e.g. via `recruiter-outreach-check-inbox` on a cron) to:
  ...
  - detect replies from recruiters previously emailed, so the follow-up
    scheduler stops nudging them
  - detect 'unsubscribe' replies and suppress those addresses

This closes what was previously the biggest gap: sends were fire-and-forget,
with no way to know who bounced or replied.
"""
...
BOUNCE_SUBJECT_HINTS = (
    "undelivered", "delivery status", "returned mail", "failure notice", "delivery failed",
)
...
class InboxTracker:
    ...
    def _looks_like_bounce(from_addr: str, subject: str) -> bool:
        ...
        return any(h in f for h in BOUNCE_SENDER_HINTS) or any(h in s for h in BOUNCE_SUBJECT_HINTS)
    ...
    def _extract_bounced_address(msg, known_emails: set[str]) -> str | None:
        """Bounce formats vary a lot between providers, so this scans the
        whole body for any address we recognise as a prior recipient."""
        body = _get_body_text(msg)

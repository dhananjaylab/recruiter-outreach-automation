# FILE: src/recruiter_outreach/verification/email_verifier.py
...
Two tiers:
  ...
  2. SMTP RCPT probe (best-effort — most consumer/office networks block
     ...
     than reject at RCPT time, so treat this as a bonus signal, not a
     guarantee. Default OFF.)
"""
...
def smtp_rcpt_check(email: str, helo_domain: str = "example.com", timeout: int = 8) -> bool | None:
    """
    Opens an SMTP connection to the recipient domain's MX and issues
    RCPT TO without sending DATA, to gauge whether the mailbox is likely
    to accept mail.
